...
class ReceptionCommittee(models.Model):
    ...
    # Relations avec d'autres modèles - seront définies dans les modules correspondants
    # purchase_order_ids = fields.One2many('e_gestock.purchase_order', 'committee_id', string='Bons de commande assignés')
    # reception_ids = fields.One2many('e_gestock.reception', 'committee_id', string='Réceptions')
    ...
    def action_view_purchase_orders(self):
        ...
        return {
            ...
            'res_model': 'purchase.order',  # Temporairement remplacé par purchase.order
            'view_mode': 'list,form',
            'domain': [],
            'context': {},
        }
    ...
    def action_view_receptions(self):
        ...
        return {
            'name': _('Réceptions'),
            'type': 'ir.actions.act_window',
            'res_model': 'stock.picking',  # Temporairement remplacé par stock.picking
            'view_mode': 'list,form',
            'domain': [],
        }

    def action_view_pvs(self):
        """Affiche les PV de réception liés à ce comité"""
        self.ensure_one()
        return {
            'name': _('Procès-verbaux'),
            'type': 'ir.actions.act_window',
            'res_model': 'mail.message',  # Temporairement remplacé par mail.message
            'view_mode': 'list,form',
            'domain': [],
        }
